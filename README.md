...
| **双层记忆** | **v3.3**：Redis 短期记忆（滑动窗口 + 批压缩摘要）+ 长期记忆（关键词/会话结束抽取 → PG + 向量召回），跨会话记住你是谁、聊过什么 |
| **用户画像 + 上下文分层** | **v3.3**：画像 / 长期记忆 / 早期摘要按 7 层结构注入 prompt，CJK 加权 Token 预算控制，长对话不爆 context；记忆 LLM 调用全部复用会话模型（BYOK） |
| **记忆管理页** | **v3.3**：`/memories` 查看 / 编辑 / 删除长期记忆 + 类型筛选 + 统计；会话可手动「结束并提取记忆」；每日凌晨自动去重 / 衰减 / 淘汰 |
...
| **Docker 化部署** | 5 容器 compose（postgres + **redis** + backend + frontend + nginx），`./scripts/deploy.sh` 一行起 |
...
│   │   ├── conversations/         v2-M3: 跨设备会话历史 + v3.3 记忆子系统
│   │   │   ├── models.py          Conversation / Message / UserMemory (v3.3)
│   │   │   ├── routes.py          /api/conversations/*（含 v3.3 finalize）+ bulk import + export
│   │   │   ├── memory_routes.py   v3.3: /api/memories CRUD + 统计
│   │   │   ├── short_term_memory.py   v3.3: Redis 窗口 + 会话 LLM 批压缩（CAS 写回）
│   │   │   ├── long_term_memory.py    v3.3: 记忆抽取 / 画像聚合 / L2 召回
│   │   │   ├── keyword_extractor.py   v3.3: 五类关键词正则实时命中
│   │   │   └── memory_maintenance.py  v3.3: 每日去重/衰减/淘汰 + 24h 自动抽取
...
│   │   │   ├── context_builder.py v3.3: 7 层上下文 + Token 预算 + tool-pair 安全截断
│   │   │   ├── state.py           AgentState TypedDict
│   │   │   └── prompts.py         3 套 system prompt + build_context_sections (v3.3)
│   │   ├── tools/                 Agent 工具实现
...
│   │       ├── redis_client.py    v3.3: 可选 Redis 客户端（熔断降级，故障不破坏 chat）
│   │       ├── memory_vector.py   v3.3: 用户记忆向量 collection（降级优先包装）
│   │       └── rate_limit.py      内存 sliding window
│   ├── data/                      运行时数据（gitignored）
│   │   ├── app.db                 SQLite 应用库
│   │   ├── milvus_local.db        Milvus Lite 向量库
│   │   └── uploads/{kb_id}/       原始上传文件
│   ├── tests/                     188 例全量：v3.3 记忆全套（context_builder + 短期/长期/关键词/维护/路由）+ test_admin (28) + smoke + conftest
...
│   │   ├── memories/              v3.3: 记忆管理页（类型筛选 / 编辑 / 删除 / 统计）
...
│   │   ├── conversations-api.ts   会话 CRUD wrapper（含 v3.3 finalize）
│   │   ├── memories-api.ts        v3.3: 记忆列表 / 编辑 / 删除 wrapper
...
| `conversations/` | 跨设备会话历史 + **v3.3 双层记忆子系统**（短期压缩 / 长期抽取 / 画像 / 每日维护 / /api/memories） | `routes.py`, `short_term_memory.py`, `long_term_memory.py`, `memory_maintenance.py` |
| `settings_user/` | 每用户自助配 LLM / Embedding / Reranker，含 probe + Fernet 加密 | `routes.py`, `probe.py` |
| `agent/` | LangGraph 主循环 + 3 套 system prompt + 三态路由 + **v3.3 7 层上下文 / Token 预算** | `graph.py`, `nodes.py`, `context_builder.py` |
...
| `app/memories/` | **v3.3** 记忆管理页（类型筛选 chips / 分页 / 行内编辑 Dialog / 删除确认 / 统计徽标条），Sidebar 用户菜单「我的记忆」进入 |
...
### 双层记忆系统（v3.3 / 2026-07 大更新）

> Agent 从"每次全量平铺历史"升级为**分层上下文 + 短期/长期双层记忆**。
> 所有记忆相关 LLM 调用（压缩 / 抽取 / 格式化）一律复用**用户为当前会话配置的模型**（BYOK + 会话级 override），不引入任何平台方模型。

**7 层上下文**（`agent/context_builder.py`，每次 plan 前拼装）：

| 层 | 内容 | 数据来源 | 预算 (token) |
|---|---|---|---|
| L0 | 系统定义 | `prompts.py`（按模式） | 400，不可截断 |
| L1 | 用户画像 | Redis 热缓存 → PG 聚合 | 200，不可截断 |
| L2 | 长期记忆 top-3 | 向量 ANN → PG importance 兜底 | 300，不可截断 |
| L4 | 早期对话摘要 | Redis → PG（批压缩产物） | 300，不可截断 |
| L5 | 最近消息窗口 | 滑动窗口（默认 10 轮） | 3000，可缩 |
| L6 | 工具结果 | 运行时动态追加 | 1500，可截 |

Token 估算做了 CJK 加权（中文 ≈1.5 字/token）；截断只落在完整对话轮边界，绝不拆散 `tool_use`/`tool_result` 配对；总预算 `CONTEXT_TOTAL_BUDGET=8000`（0 = 不启用预算）。空层整段省略，匿名请求跳过 L1/L2。

**短期记忆**：消息照旧全量写 PG 永久存档；Redis List 另存最近窗口（LPUSH+LTRIM，3 天 TTL）。每积累 5 轮（可配）用会话 LLM 把最老一批压成 ≤150 字中文摘要，线性拼接进 L4。压缩写回走 CAS（`UPDATE ... WHERE compressed_count = 快照值`），多 worker 并发安全。

**长期记忆**：三个来源写入 `user_memories` 表 + `user_memory_vectors` 向量 collection——
1. **实时关键词**：用户消息命中五类正则（我是 / 我喜欢 / 我们项目用 / 搞定了 / 帮我记住）→ 后台会话 LLM 格式化为 `{type, content, importance}`；
2. **会话结束抽取**：手动「结束会话」（`POST /finalize`，CAS 幂等）或 24h 未活跃自动扫描（每轮 ≤20 条），整段对话由会话 LLM 提取记忆数组；
3. **用户画像**：`profile/preference/skill/fact` 记忆行纯代码聚合（角色 / 偏好 / 技能 / 环境），Redis Hash 30 天热缓存，写入即失效重建。

L2 召回复用**用户自己的 BYOK embedding** 检索；未配 embedding 或向量库故障时降级为 PG importance 排序。所有读路径按鉴权 user_id 三重隔离（向量 filter + PG WHERE + Redis key 命名空间）。

**每日维护**（UTC `MEMORY_MAINTENANCE_HOUR`，默认 3 点）：向量去重（cosine > 0.85 保留 importance 最高，每轮 ≤200 条）、衰减（30 天未访问 ×0.9，幂等门保证每天最多一次）、淘汰（importance < 0.3 软删 + 向量清理）。`explicit` 类型（用户显式要求记住）豁免衰减与淘汰。

**记忆 API**：

```http
GET    /api/memories?type=&limit=&offset=   # 列表 + 分页 + 类型筛选 + stats.by_type 统计
PATCH  /api/memories/{id}                   # 编辑 content / importance（自动重建向量）
DELETE /api/memories/{id}                   # 删除（PG + 向量连锁）
POST   /api/conversations/{id}/finalize     # 结束会话并提取记忆（幂等，返回提取条数）
```

**降级承诺**：Redis / 向量库 / embedding 任一未配置或故障时逐层降级（热缓存 → PG、向量召回 → importance 排序、压缩/抽取跳过）；`REDIS_URL` 留空则短期记忆热路径整体关闭、行为回到 v3.2——**chat 主流程永不因记忆系统 500**。
...
> **v3.3 迁移记录**（全自动，无需手工干预）：`conversations` 加列 `compressed_count` / `compression_watermark` / `context_summary` / `finalized_at`；新表 `user_memories`（11 列，含软删 `deleted_at` 与衰减幂等门 `last_decayed_at`，复合索引 `(user_id, memory_type)`）由 `create_all` 自动创建。首次启动新版本时由 `init_db` 幂等完成——线上生产 PostgreSQL 已于 2026-07-18 验证通过。
...
(:5432)         (:8000)         (:3000)
   ▲                │ ▲
   │        asyncpg │ │ redis://redis:6379 (v3.3 记忆热存储)
   └────────────────┘ └──► redis (:6379)
```
...
2026-05-25 已从 systemd 切换到 Docker。架构 = §6 的 5 服务全栈（2026-07-18 v3.3 起新增 redis）：
...
| **记忆系统 (v3.3)** | `REDIS_URL` | (空)=关闭 / `redis://redis:6379/0` | 短期记忆热存储总开关；Docker compose 自动注入，留空回到 v3.2 行为 |
| | `MEMORY_WINDOW_SIZE` | `10` | 最近消息窗口（轮数），`0`=全量保留不压缩 |
| | `MEMORY_COMPRESSION_BATCH` | `5` | 每积累 N 轮压缩一批为摘要（会话 LLM） |
| | `CONTEXT_TOTAL_BUDGET` | `8000` | 上下文总 token 预算，`0`=不启用 |
| | `MEMORY_AUTO_EXTRACT` | `true` | 自动抽取长期记忆（关键词 + 会话结束 / 24h 扫描） |
| | `MEMORY_MAINTENANCE_ENABLED` | `true` | 每日维护任务（去重 / 衰减 / 淘汰）总开关 |
| | `MEMORY_MAINTENANCE_HOUR` | `3` | 维护时刻（UTC 小时） |
...
```bash
cd backend
uv run pytest tests/ -q         # 188 例全量（v3.3 记忆系统 + admin + smoke）
uv run pytest -q -k "memory or context"   # 只跑记忆 / 上下文相关
```
